using Rotation.UV;

namespace Rotation;

public record Tangent(Vector T, Vector B, Vector N);

public class Triangle {
    public Triangle(int pIdx, Wrapper<Vector>[] pVertices) {
        if (pVertices.Length < 3) throw new ArgumentOutOfRangeException("pVertices count must over 3");
        Idx = pIdx;
        A = pVertices[0];
        B = pVertices[1];
        C = pVertices[2];
        NormalUpdate();
    }

    public readonly int Idx;
    public Wrapper<Vector> A { get; set; }
    public Wrapper<Vector> B { get; set; }
    public Wrapper<Vector> C { get; set; }
    public Vector Normal { get; private set; }
    public Tangent Tangent { get; private set; } = new(default, default, default);
    public Vector U => B.V - A.V;
    public Vector V => C.V - A.V;
    public Vector Middle => (A.V + B.V + C.V) / 3f;

    public bool NeedTangentUpdate() =>
        Normal != Tangent.N;
    
    public void TangentUpdate(UVMap.UVCoord pUa, UVMap.UVCoord pUb, UVMap.UVCoord pUc) {
        var ba = U;
        var ca = V;
        var uba = pUb - pUa;
        var uca = pUc - pUa;
        var det = uba.U * uca.V - uba.V * uca.U;
        var t = ((ba * uca.V - ca * uba.V) / det).Normalized;
        //May be opengl normalmap issue, but I'm using right hand coordinate.
        //but just on calculate it, b axis is flipped. hmm... It works successful. bbb
        var b = -((ba * uca.U - ca * uba.U) / det).Normalized;
        Tangent = new(t, b, Normal);
    }
    
    public void NormalUpdate() {
        Normal = U.Cross(V).Normalized;
    }
	
    public Vector GetPoint(float pU, float pV) =>
        (1 - pU - pV) * A.V + pU * B.V + pV * C.V;

    /*
    public void RotateX(float pDelta) {
        var middle = Middle;
        A.V.RotateX(middle, pDelta);
        B.V.RotateX(middle, pDelta);
        C.V.RotateX(middle, pDelta);
    }
    public void RotateY(float pDelta) {
        var middle = Middle;
        A.V.RotateY(middle, pDelta);
        B.V.RotateY(middle, pDelta);
        C.V.RotateY(middle, pDelta);
    }
    public void RotateZ(float pDelta) {
        var middle = Middle;
        A.V.RotateZ(middle, pDelta);
        B.V.RotateZ(middle, pDelta);
        C.V.RotateZ(middle, pDelta);
    }

    public void Rotate(float pX, float pY, float pZ) {
        RotateX(pX);
        RotateY(pY);
        RotateZ(pZ);
    }*/
}